...
async function getPredefinedTemplates() {
	...
	try {
		...
		console.log("🔑 Using auth token:", AUTH_TOKEN ? `${AUTH_TOKEN.substring(0, 8)}...` : 'Not set');
		...
		if (!response.ok) {
			console.warn("⚠️ Could not fetch predefined templates, using custom template");
			return null;
		}
...
export async function POST(request: NextRequest) {
	...
	try {
		...
		console.log(`📋 Processing invoice collection: ${collectionId || 'dynamic'}`);
