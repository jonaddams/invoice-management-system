		...
		<div className="min-h-screen" style={{ background: "var(--background)" }}>
			...
			<div className="max-w-6xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
				...
				<div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
					...
					>
						<FileText className="mx-auto h-8 w-8 mb-2" style={{ color: "var(--disc-pink)" }} />
						<div className="text-2xl font-bold" style={{ color: "var(--foreground)" }}>
							{loading ? '...' : invoices.length}
						</div>
						<div className="text-sm" style={{ color: "var(--foreground)", opacity: 0.7 }}>Total Invoices</div>
					</div>
					<div
						className="rounded-lg p-6 text-center border"
						style={{
					...
					>
						<File className="mx-auto h-8 w-8 mb-2" style={{ color: "var(--code-coral)" }} />
						<div className="text-2xl font-bold" style={{ color: "var(--foreground)" }}>
							{loading ? '...' : 'PDF'}
						</div>
						<div className="text-sm" style={{ color: "var(--foreground)", opacity: 0.7 }}>File Format</div>
					</div>
					<div
						className="rounded-lg p-6 text-center border"
						style={{
					...
					>
						...
						<div className="text-sm" style={{ color: "var(--foreground)", opacity: 0.7 }}>Total Size</div>
					</div>
					<div
						className="rounded-lg p-6 text-center border"
						style={{
					...
					>
						...
						<div className="text-sm" style={{ color: "var(--foreground)", opacity: 0.7 }}>Status</div>
					</div>
				</div>
				...
				>
					...
					>
						<h2 className="text-lg font-semibold" style={{ color: "var(--foreground)" }}>Invoice Documents</h2>
					</div>
					<div style={{ borderColor: "var(--neutral)" }}>
						{loading ? (
							<div className="px-6 py-12 text-center">
								<RefreshCw className="h-8 w-8 animate-spin mx-auto mb-4" style={{ color: "var(--neutral)" }} />
								<p style={{ color: "var(--foreground)", opacity: 0.7 }}>Loading invoices...</p>
							</div>
						) : error ? (
							<div className="px-6 py-12 text-center">
								<p className="mb-2" style={{ color: "var(--code-coral)" }}>Error: {error}</p>
								<p style={{ color: "var(--foreground)", opacity: 0.7 }}>Please check that PDF files are present in /public/documents/invoices/</p>
							</div>
						) : invoices.length === 0 ? (
							<div className="px-6 py-12 text-center">
								<File className="h-12 w-12 mx-auto mb-4" style={{ color: "var(--neutral)" }} />
								<p style={{ color: "var(--foreground)", opacity: 0.7 }}>No invoice files found</p>
				...
				<div className="mt-16 text-center text-sm" style={{ color: "var(--neutral)" }}>
